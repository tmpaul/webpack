...
exports.isExcludedFromSharing = (context, request, excludeConfig) => {
	if (Array.isArray(excludeConfig)) {
		return (
			excludeConfig.find(e =>
				this.isExcludedFromSharing(context, request, e)
			) !== undefined
		);
	}
	if (typeof excludeConfig === "string") {
		return context === excludeConfig;
	}
	if (excludeConfig instanceof RegExp) {
		return excludeConfig.test(context);
	}
	if (typeof excludeConfig === "function") {
		return excludeConfig(context, request);
	}
	return false;
};
