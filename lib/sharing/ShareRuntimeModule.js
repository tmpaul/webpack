			...
			`${RuntimeGlobals.initializeSharing} = ${runtimeTemplate.basicFunction(
				...
				[
					...
					`var register = ${runtimeTemplate.basicFunction(
						"name, version, factory, eager, { prefer }",
						[
							"var versions = scope[name] = scope[name] || {};",
							"var activeVersion = versions[version];",
							`const retainScopeValue = !!prefer ? ((av) => prefer.find(pc => pc.test ? pc.test(av.from) : pc === av.from) || (av.from > uniqueName)) : (av) => av.from > uniqueName`,
							`if(!activeVersion || (!activeVersion.loaded && (!eager != !activeVersion.eager ? eager : !retainScopeValue(activeVersion)))) versions[version] = { get: factory, from: uniqueName, eager: !!eager };`
						]
					)};`,
